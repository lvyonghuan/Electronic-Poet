...
int Start() {
	for(;;) {
		...
		//初始化存储模块
		int isFailStore=0;
		poetry *poetry_tail=NULL;
		poetry *poetry_head=InitPoetryStore();
		poetry_tail=poetry_head;
		if(poetry_head==NULL){
			isFailStore=1;
		...
		for (int i = 0; i < l; i++) {
			...
			int err = SpliceSentence(sentence,isFailStore,&poetry_tail);
			if (err) {
				return 1;
			}
		}
...
int SpliceSentence(char *sentence,int isFailStore,poetry **tail){
	...
	printf("\n");
	if(!isFailStore){
		poetry *node=(poetry*) malloc(sizeof (poetry));
